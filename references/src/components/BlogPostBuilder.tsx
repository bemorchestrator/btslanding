...
import { ArrowLeft, Plus, Trash2, Image as ImageIcon, Type, AlignLeft, Quote, Eye, Save } from 'lucide-react';
...
import type { Article, ArticleContent, Category } from '../App';
...
export function BlogPostBuilder({ article, categoryId, categories, onSave, onCancel }: BlogPostBuilderProps) {
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Select
                ...
                onValueChange={(value: any) => updateBlockStyle(index, { textAlign: value })}
              >
  ...
  const renderBlockPreview = (block: ArticleContent, index: number) => {
        ...
        <TabsContent value="preview" className="mt-4">
          <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-8">
            ...
            <div className="prose prose-invert max-w-none">
              {contentBlocks.map((block, index) => (
                <div key={index}>{renderBlockPreview(block, index)}</div>
              ))}
            </div>
          </div>
        </TabsContent>
